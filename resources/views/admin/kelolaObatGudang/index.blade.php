            ...
            $(document).on('click', '#btn-delete-obatGudang', function() {
                if (currentObatGudangId) {
                    Swal.fire({
                        ...
                        text: 'Data obat ke gudang ini akan dihapus secara permanen!',
                        icon: 'warning',
                        ...
                        if (result.isConfirmed) {
                            var deleteUrl = '{{ route('admin.kelolaObatGudang.destroy', ':id') }}'
                                .replace(':id', currentObatGudangId);
                            $.ajax({
                                url: deleteUrl,
                                type: 'POST',
                                data: {
                                    ...
                                    if (response.success) {
                                        Swal.fire({
                                            ...
                                            window.location.href =
                                                "{{ route('admin.kelolaObatGudang.index') }}"; // Redirect ke index
                                        });
                                    } else {
                                        Swal.fire({
                                            title: 'Gagal!',
                                            text: 'Gagal menghapus obat ke gudang: ' +
                                                response.message,
                                            icon: 'error'
                                        });
                                    }
                                ...
                                error: function(xhr) {
                                    let errorMsg = 'Gagal menghapus obat ke gudang.';
                    ...
                    Swal.fire({
                        title: 'Error!',
                        text: 'ID obat ke gudang tidak ditemukan',
                        icon: 'error'
                    });
                }
            });
